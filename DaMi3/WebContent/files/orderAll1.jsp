...
function showlist(n)
{
	 ...
	 $.ajax({
		     ...
		     success:function(data){
                ...
                +"<td width='8%' height='20' bgcolor='#FFFFFF' class='STYLE19'><div align='center'><span class='STYLE10'><a href='indentinfo2?indentid="+this.id+"'>查看订单详情</a>|<a onclick='ss("+this.id+")'>发货</a>|"
                +"<a onclick='ss("+this.id+")'>签收</a>|<a onclick='sss("+this.id+")' >关闭订单</a>|<a onclick='sss("+this.id+")'>关闭</a>"+"</span></div></td></tr>");
                if(this.info == 0){
               	 $("#tds").html("未发货");
                }
                if(this.info == 1){
...
$(function(){
	$("#select").click(function(){
		...
		$.ajax({
			url:"all_findall",
			type:"POST",
			data:{"indent.info":0,"indent.totalprices":$("#select1").val()},
			success:function(data){
				$("#page tr").remove();
				$("#yan~tr").remove();
		    	 $.each(data,function(){
